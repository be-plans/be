...
pkg_deps=(core/glibc lilian/xz lilian/zlib)
pkg_build_deps=(lilian/make lilian/gcc lilian/pkg-config lilian/file lilian/sed lilian/diffutils)
pkg_lib_dirs=(lib)
...
do_before() {
  if [[ ! -f /usr/bin/file ]]; then
    hab pkg binlink lilian/file file -d /usr/bin
    _clean_file=true
  fi
}
