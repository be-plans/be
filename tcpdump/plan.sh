...
# be/coreutils isn't /really/ needed at runtime, but fix_interpreter
# only works if the dep is listed in pkg_deps
pkg_deps=(
  core/glibc lilian/libpcap be/openssl
  be/coreutils
)
pkg_build_deps=(
...
do_check() {
  fix_interpreter "tests/TESTonce" be/coreutils bin/env
  make check
}
